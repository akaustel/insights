...
function Insight(div, data, options) {
    ...
    // Menu container
    // Visiblity buttons
    let visibilityPivot = document.createElement('button');
    visibilityPivot.textContent = 'Pivot';
    visibilityPivot.onclick = (function (obj) { return function () { obj.options.pivot.visible = !obj.options.pivot.visible; obj.render(); }; })(this);
    let visibilityTable = document.createElement('button');
    visibilityTable.textContent = 'Table';
    ...
    document.getElementById(div).append(this.menu_container, this.settings_container, this.pivot_container, this.table_container);
    this.render = function () {
        // Filters
        this.rows = [];
        if (this.options.filters.master != "") {
            for (let i = 0; i < this.data.length; i++) {
                if (this.data[i].join().toLowerCase().includes(this.options.filters.master)) {
    ...
    this.renderPivot = function () {
        ...
        if (!this.options.pivot.visible) {
            this.pivot_container.style.display = 'none';
            return;
        }
        ...
        if (this.options.pivot.rows.length == 0 && this.options.pivot.cols.length == 0) // Nothing selected
         {
            ...
            let total_grand_label = document.createElement("th");
            total_grand_label.textContent = 'Totals';
            let total_grand_value = document.createElement("th");
            total_grand_value.textContent = total_grand;
        ...
        else if (this.options.pivot.rows.length > 0 && this.options.pivot.cols.length == 0) // Rows
         {
            ...
            for (let i = 0; i < this.options.pivot.rows.length; i++) {
                ...
                header.textContent = this.headers[this.options.pivot.rows[i]];
            ...
            let total_grand_label = document.createElement("th");
            total_grand_label.textContent = 'Totals';
            total_grand_label.colSpan = this.options.pivot.rows.length;
            let total_grand_value = document.createElement("th");
            total_grand_value.textContent = total_grand;
            tr.append(total_grand_label, total_grand_value);
            tfoot.append(tr);
        ...
        else if (this.options.pivot.rows.length == 0 && this.options.pivot.cols.length > 0) // Cols
         {
            ...
            for (let i = 0; i < this.options.pivot.cols.length; i++) {
                ...
                name.textContent = this.headers[this.options.pivot.cols[i]];
                tr.append(name);
        ...
        else // Rows & Cols
         {
            ...
            for (let i = 0; i < this.rows.length; i++) {
                ...
                let matrix_keys = [];
                for (let i2 = 0; i2 < this.options.pivot.cols.length; i2++) {
                    col_keys.push(this.rows[i][this.options.pivot.cols[i2]]);
                }
                for (let i2 = 0; i2 < this.options.pivot.rows.length; i2++) {
                    row_keys.push(this.rows[i][this.options.pivot.rows[i2]]);
                }
                let col_key = col_keys.join('|');
                let row_key = row_keys.join('|');
                let matrix_key = row_key + '|' + col_key;
                if (typeof colMap[col_key] == 'undefined') {
                    colMap[col_key] = [col_key, col_keys, 1];
                }
                else {
                    colMap[col_key][2]++;
                } // Count
                if (typeof rowMap[row_key] == 'undefined') {
                    rowMap[row_key] = [row_key, row_keys, 1];
                }
                else {
                    rowMap[row_key][2]++;
                } // Count
                if (typeof matrix[matrix_key] == 'undefined') {
                    matrix[matrix_key] = 1;
                }
                else {
                    matrix[matrix_key]++;
            ...
            let type = 0;
            let order = true;
            if (this.options.pivot.sort.cols.type == 'total') {
                type = 2;
            }
            else {
                type = 0;
            }
            if (this.options.pivot.sort.cols.order) {
                cols.sort(function (a, b) { return a[type] > b[type] ? 1 : -1; });
            }
            else {
                cols.sort(function (a, b) { return a[type] < b[type] ? 1 : -1; });
            }
            if (this.options.pivot.sort.rows.type == 'total') {
                type = 2;
            }
            else {
                type = 0;
            }
            if (this.options.pivot.sort.rows.order) {
            ...
            for (let i = 0; i < this.options.pivot.cols.length; i++) {
                ...
                // Label click sorting
                label.onclick = (function (obj) {
                    return function () {
                        if (obj.options.pivot.sort.cols.type == 'label') {
                            obj.options.pivot.sort.cols.order = !obj.options.pivot.sort.cols.order;
                        }
                        else {
                            obj.options.pivot.sort.cols.type = 'label';
                            obj.options.pivot.sort.cols.order = true;
                        }
                        obj.render();
                    };
                })(this);
                ...
                for (let i2 = 0; i2 < cols.length; i2++) {
                    let label = document.createElement("th");
                    label.textContent = cols[i2][1][i];
                    if ((i + 1) == this.options.pivot.cols.length) {
                        label.rowSpan = 2;
                    }
                    tr.append(label);
                }
                if (i == 0) {
                    ...
                    label.onclick = (function (obj) {
                        return function () {
                            if (obj.options.pivot.sort.rows.type == 'total') {
                                obj.options.pivot.sort.rows.order = !obj.options.pivot.sort.rows.order;
                            }
                            else {
                                obj.options.pivot.sort.rows.type = 'total';
                                obj.options.pivot.sort.rows.order = true;
                            }
                            obj.render();
                        };
                    })(this);
            ...
            for (let i = 0; i < this.options.pivot.rows.length; i++) {
                ...
                label.onclick = (function (obj) {
                    return function () {
                        if (obj.options.pivot.sort.rows.type == 'label') {
                            obj.options.pivot.sort.rows.order = !obj.options.pivot.sort.rows.order;
                        }
                        else {
                            obj.options.pivot.sort.rows.type = 'label';
                            obj.options.pivot.sort.rows.order = true;
                        }
                        obj.render();
                    };
            ...
            label.onclick = (function (obj) {
                return function () {
                    if (obj.options.pivot.sort.cols.type == 'total') {
                        obj.options.pivot.sort.cols.order = !obj.options.pivot.sort.cols.order;
                    }
                    else {
                        obj.options.pivot.sort.cols.type = 'total';
                        obj.options.pivot.sort.cols.order = true;
                    }
                    obj.render();
                };
    ...
    this.renderTable = function () {
        ...
        if (!this.options.table.visible) {
            this.table_container.style.display = 'none';
            return;
        }
        let table = document.createElement("table");
        let thead = document.createElement("thead");
        ...
        if (sort_order) {
            this.rows.sort(function (a, b) { if (!isNaN(a[sort_col]) && !isNaN(b[sort_col])) {
                return Number(a[sort_col]) > Number(b[sort_col]) ? 1 : -1;
            }
            else {
        ...
        for (let i = 0; i < this.headers.length; i++) {
            ...
            // Sorting handler
            th.onclick = (function (obj) {
                return function () {
                    if (obj.options.table.sort.col == i) {
                        obj.options.table.sort.order = !obj.options.table.sort.order;
                    }
                    else {
                        obj.options.table.sort.order = true;
                        obj.options.table.sort.col = i;
                    }
                    obj.render();
                };
            })(this);
            // Label content
            if (this.options.table.sort.col == i && this.options.table.sort.order) {
                th.textContent = this.headers[i] + ' ↑';
            }
            else if (this.options.table.sort.col == i && !this.options.table.sort.order) {
                th.textContent = this.headers[i] + ' ↓';
            }
            else {
                th.textContent = this.headers[i];
